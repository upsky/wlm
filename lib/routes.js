...
var openUrls = ['login', 'error', 'down', 'loading', 'blocked', 'reg', 'qr', 'recoverpass', 'regemail', 'logout'];
		...
		if (!flag)
			if (Router.current().route.getName() !== 'profile') {
				...
				noticeError = new PNotify({
					text: 'Email не подтвержден, перейдите в <a href="/profile">профиль</a> для подтверждения.',
					type: 'warning',
					hide: false,
